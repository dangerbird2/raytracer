...
void matmult(float *in, float *out, float mat[4][4]) {
  float tmp[3];
  tmp[0] = (in[0] * mat[0][0] + in[1] * mat[0][1] + in[2] * mat[0][2]);
  tmp[1] = (in[0] * mat[1][0] + in[1] * mat[1][1] + in[2] * mat[1][2]);
  tmp[2] = (in[0] * mat[2][0] + in[1] * mat[2][1] + in[2] * mat[2][2]);
  vcopy(tmp, out);
  return;
}
...
void matcopy(float a[4][4], float b[4][4]) { memcpy(a, b, 16 * sizeof(float)); }
...
static float det2x2(float a1, float b1, float a2, float b2) {
  return (a1 * b2 - b1 * a2);
}

static float det3x3(float a1, float b1, float c1, float a2, float b2, float c2,
                    float a3, float b3, float c3) {
  return (a1 * det2x2(b2, c2, b3, c3) - b1 * det2x2(a2, c2, a3, c3) +
...
float matadjoint(float r[4][4], float _data[4][4]) {
  ...
  outDet = a1 * r[0][0] + b1 * r[1][0] + c1 * r[2][0] + d1 * r[3][0];
